<?php
if (preg_match('#^http://(www\.)?filereactor\.com/#', $url)){
	$account = trim($this->get_account('filereactor.com'));
	if (stristr($account,':')) list($user, $pass) = explode(':',$account);
	else $cookie = $account;
	if(empty($cookie)==false || ($user && $pass)){
		for ($j=0; $j < 2; $j++){
			if(!$cookie) $cookie = $this->get_cookie("filereactor.com");
			if(!$cookie){
				$data = $this->curl("http://www.filereactor.com/","","op=login&redirect=&login=$user&password=$pass");
				$cookie = $this->GetCookies($data);
				$this->save_cookies("filereactor.com",$cookie);
			}
			$this->cookie = $cookie;
			...
			elseif (preg_match('%input type="hidden" name="op" value="(.*)"/>%U', $data, $redir2)) {
				...
				if (preg_match('%<a title="Download link" href="(http:\/\/.+\.filereactor\.com.+)">%U', $data, $redir2)) $link = trim($redir2[1]);
			}
			if($link){
				...
				$cookie = "";
				$this->save_cookies("filereactor.com","");
			}
		}
	}
}
