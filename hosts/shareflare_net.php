<?php
if (preg_match('#^http://www.shareflare.net/#', $url) || preg_match('#^http://shareflare.net/#', $url)){
	$account = trim($this->get_account('shareflare.net'));
	if (stristr($account,':')) list($user, $pass) = explode(':',$account);
	else $cookie = $account;
	if(empty($cookie)==false || ($user && $pass)){
		for ($j=0; $j < 2; $j++){
			if(!$cookie) $cookie = $this->get_cookie("shareflare.net");
			if(!$cookie) {
				$data = $this->curl("http://shareflare.net/","lang=en","act=login&login=".urlencode($user)."&password=$pass");
				$cookie = $this->GetCookies($data);
				$this->save_cookies("shareflare.net",$cookie);
			}
			if (stristr($cookie,'prekey')) {
				...
				$data = $this->curl("http://shareflare.net/sms/check2.php","",$post);
